...
public interface ForumService {
    ...
    PageResult<ThreadVO> getThreadsByBoardPaged(Integer boardId, String keyword, String threadType, String tagName, int pageNum, int pageSize);
    PageResult<ThreadVO> searchThreads(String keyword, int pageNum, int pageSize);
    PageResult<ThreadVO> searchThreads(String keyword, Integer boardId, String threadType, String tagName, int pageNum, int pageSize);
    List<ThreadVO> getHotThreads(int limit);
    List<ThreadVO> getLatestThreads(int limit);
    List<ThreadVO> getEssenceThreads(int limit);
    List<ThreadVO> getActivityThreads(int limit);
    
    // 帖子详情页面数据：主题、增加浏览量、各楼层及其楼中楼
    ThreadVO getThreadWithViewCountInc(Long threadId);
    List<PostVO> getPostsWithReplies(Long threadId);
    List<ThreadImageVO> getThreadImages(Long threadId);
    
    // 核心交互：发帖
    Long createThread(Integer boardId, Long userId, String title, String content);
    Long createThread(Integer boardId,
                      Long userId,
                      String title,
                      String content,
                      String threadType,
